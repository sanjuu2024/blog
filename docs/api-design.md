...
| `102009` | `409` | `隐私政策已更新，请重新打开隐私政策页面` | 注册请求提交的隐私政策版本不是当前后端版本 |
...
| `privacyPolicyVersion` | `String` | 是 | 当前隐私政策版本，必须是前端从 `/api/v1/privacy-policy` 获取的 `sha256:` 内容哈希 | `sha256:012345...` |

说明：注册时无需传入 `nickname`，服务端默认使用 `username` 初始化昵称，后续用户可在个人中心修改昵称。服务端会比较 `privacyPolicyVersion` 与当前隐私政策版本；版本不一致时返回 `102009`，前端应提示隐私政策已更新并重新打开隐私政策弹窗。
...

{
  ...
  "password": "Passw0rd!",
  "privacyPolicyVersion": "sha256:0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef"
}
```
...
若 `privacyPolicyVersion` 不是当前版本，返回 HTTP `409`、业务码 `102009`，提示“隐私政策已更新，请重新打开隐私政策页面”。
