...
| 项目                  | 记录                                                         |
| --------------------- | ------------------------------------------------------------ |
| 验收版本 / Commit SHA | 18d3333bdad56c275e3d9f99d5ff5bed55ae5be6                     |
| 验收分支              | `develop`                                                    |
| 验收开始时间          | 2026-09-16                                                   |
| 验收完成时间          | 2026-09-25                                                   |
| 验收负责人            | sanjuu                                                       |
...
| 最终结论              | 本机 P1 功能与生产式部署演练完成；正式公网部署留待上线准备阶段。 |
...
| P1-004 | Major | CI / 测试配置 | 发现版本 `87c1b21` 存在问题：<br />测试 profile 未加载 `db/devdata`，导致 CI 的 `ArticleSearchIntegrationTest` 在空测试数据上查询到 0 条记录。 | 处理结果：测试 profile 同时加载正式 migration 和开发测试数据。<br />修复版本：`78d6931`。 | 复测结果：CI 后端 Job 已复测通过。 |
| P1-005 | Major | 前端生产构建 | 发现版本 `5ed4dc7` 存在问题：<br />自动导入和全局组件的 TypeScript 声明文件被忽略，干净 Docker/CI checkout 执行 `vue-tsc` 时大量 `ref`、`reactive`、`ElMessage` 和隐式类型错误。 | 处理结果：将 `auto-import.d.ts`、`components.d.ts` 纳入仓库。<br />修复版本：`07ca7b8`。 | 复测结果：CI 和本地 `pnpm build` 已复测通过。 |
| P1-006 | Major | 生产前端配置 | 发现版本 `41bc912` 存在问题：<br />生产构建未加载 `VITE_API_BASE_URL`，刷新登录态请求被拼接为 `/auth/auth/refresh` 并由 Nginx 返回 `405`。 | 处理结果：新增前端生产环境 API 基础路径 `/api/v1`。<br />修复版本：`1c443f1`。 | 复测结果：HTTPS 登录、刷新和退出已复测通过。 |
| P1-007 | Major | 生产管理员 bootstrap | 发现版本 `29d5f72` 存在问题：<br />非 Web bootstrap profile 仍无条件创建 `SecurityFilterChain`，因缺少 `HttpSecurity` 启动失败。 | 处理结果：仅在非 `bootstrap-admin` profile 注册 Web 安全链，保留 `PasswordEncoder`。<br />修复版本：`3aec4fb`。 | 复测结果：bootstrap 已成功创建首个管理员。 |
| P1-008 | Minor | 生产 bootstrap 构建 | 发现版本 `2392b16` 存在问题：<br />后端 Docker 构建访问 Maven Central 时 TLS 握手不稳定，bootstrap 镜像构建失败。 | 处理结果：构建阶段配置 Maven 公共镜像。<br />修复版本：`a8db9a6`。 | 复测结果：bootstrap 镜像已成功构建并运行。 |
| P1-009 | Major | 生产管理员 bootstrap | 发现版本 `2392b16` 存在问题：<br />宿主机 `600` 权限的 bootstrap 密码文件以 root-owned Secret 挂载，而后端运行 UID `10001` 无法读取，初始化失败。 | 处理结果：bootstrap overlay 的一次性任务以 `0:0` 读取只读 Secret，长期 backend 仍以 UID `10001` 运行。<br />修复版本：本次 bootstrap 修复提交。 | 复测结果：bootstrap 已成功创建首个管理员。 |
| P1-010 | Major | OSS 图片访问 | 发现版本 `18d3333` 存在问题：<br />OSS 上传成功但自定义图片域名的 DNS/HTTPS/公开读取链路未配置完成，头像和文章封面浏览器加载失败。 | 处理结果：修正 prodtest 图片域名、DNS/CNAME、HTTPS 和公开读取配置；无后端代码变更。 | 复测结果：头像、文章封面和正文图片 URL 已通过 HTTPS 匿名读取。 |
| P1-011 | Minor | 前台文章搜索 | 发现版本 `18d3333` 存在问题：<br />标题为 `111` 的文章搜索 `1` 或 `11` 无结果，纯数字关键词未执行字面量子串兜底。 | 处理结果：纯数字关键词对标题、摘要和正文执行字面量包含匹配，并补充集成测试与文档。<br />修复版本：本次验收修复提交。 | 复测结果：自动化测试覆盖 `1`、`11` 命中 `111`；真实 CI 复测待本次提交后执行。 |
...
- [x] 本清单中的必测项目全部通过，或已按规则标记为 `[N/A]`。
- [x] GitHub Actions 前端、后端 Job 全部通过。

- [x] 不存在未解决的 `Blocker`、`Critical` 缺陷。
- [x] 核心业务不存在未解决的 `Major` 缺陷；非核心 `Major` 缺陷有明确处理结论、负责人和计划时间。
- [x] 空数据库迁移和历史数据库升级演练均通过。
- [x] 真实 OSS 与真实 SMTP 各完成至少一次外部服务验收。
- [x] 本机 Linux / WSL 生产式部署演练通过。
- [x] 代码、数据库、接口与产品文档保持一致。
...
- [x] 游客不能发表评论，登录后可以发表评论和回复。
- [x] 普通用户评论初始为待审核，管理员评论直接通过。
- [x] 登录用户可以看到自己的待审核或已拒绝评论，其他用户和游客不可见。
- [x] 携带失效 Token 请求评论列表时返回认证错误，不静默降级为游客。
- [x] 文章关闭评论后不能新增评论或回复，但已有评论仍可查看，作者仍可删除自己的评论。
- [x] 回复插入当前目标评论下方，回复目标、根评论关系和多级展示正确。
- [x] 加载更多回复不会与本地新插入回复重复，剩余数量文案正确。
- [x] 长评论可以展开、收起，切换后定位到整条评论，回复、删除和展开操作位于同一底部工具栏。
- [x] 用户可以逻辑删除自己的评论，不能删除其他用户评论。
- [x] 同一用户对同一文章的评论限流生效，限流窗口结束后可以再次提交。

### 8.2 后台评论管理

- [x] 管理员可以按文章、用户、状态、层级和创建时间筛选评论。
- [x] 评论状态按 `PENDING`、`APPROVED`、`REJECTED`、`HIDDEN`、`DELETED` 的规则流转。
- [x] 拒绝、隐藏、删除必须填写处理原因；通过时清空旧原因。
- [x] 管理员可以隐藏或删除自己的评论，管理员自己的新评论直接通过。
- [x] 删除顶层评论或回复时，规定的子树逻辑删除和审核字段保留规则正确。
- [x] 审核、隐藏、删除与文章 `comment_count` 在同一事务中保持一致。
- [N/A] 批量通过只处理合法待审核评论，任一目标不合法时整批失败。（PRD 已明确延后，当前 P1 仅支持逐条审核。）
- [x] 已拒绝、已隐藏、已删除行使用 muted 视觉，已拒绝状态标签保持红色。

## 9. 留言与邮件通知

### 9.1 前台留言

- [x] 游客可以填写昵称和留言；邮箱可选，勾选通知时邮箱必填且格式合法。
- [x] 登录用户使用账号资料，可以选择使用账号邮箱接收通知。
- [x] 普通用户和游客留言初始为待审核，管理员留言直接通过。
- [x] 登录用户可以看到自己的待审核或已拒绝留言，游客刷新后不能找回未审核留言。
- [x] 携带失效 Token 请求留言列表时返回认证错误，不静默降级为游客。
- [x] 游客和登录用户的 30 秒、每小时限流均生效。
- [x] 用户只能逻辑删除自己的顶层留言，删除顶层留言时管理员回复一并逻辑删除。
- [x] 长留言可以展开、收起，底部工具栏吸附和整条留言定位正常。
- [x] 移动端留言编辑器默认折叠，展开后表单校验、提交和收起正常。

### 9.2 后台留言管理

- [x] 管理员可以按留言 ID、用户 ID、游客昵称、游客邮箱、内容、状态、层级和创建时间筛选。
- [x] 管理员可以通过、拒绝、隐藏、删除留言，处理原因和状态规则正确。
- [x] 管理员只能回复已通过的顶层留言，可以对同一留言多次回复。
- [x] 管理员回复直接通过，公共页面不向普通用户提供回复入口。
- [N/A] 批量通过最多处理 100 条待审核顶层留言，任一目标不合法时整批失败。（当前 P1 未排期批量通过。）

### 9.3 邮件通知与退订

- [x] GreenMail 集成测试验证事务提交后通过 SMTP 发送、纯文本内容和不发送分支。
- [x] 使用真实 SMTP 测试账号发送一封通知，验收邮箱实际收到邮件。
- [x] 邮件发件人、收件人、主题、留言摘要、回复内容、查看链接和退订链接正确。
- [x] 未勾选通知、邮箱为空或已经退订时不发送邮件。
- [x] 退订 GET 页面不修改状态，用户确认后的 POST 退订幂等，后续回复不再发信。
- [x] SMTP 暂时性失败最多重试 3 次，永久性认证/内容错误不重试。
- [x] 邮件最终失败不回滚已提交的管理员回复，日志不包含邮箱和留言正文。

## 10. 图片上传与 OSS

- [x] 普通用户可以上传自己的头像，管理员可以上传文章封面和正文图片。
- [x] 非管理员不能使用后台图片上传接口，未登录用户不能上传头像。
- [x] 支持 `.jpg`、`.jpeg`、PNG、WebP、GIF；SVG、伪造扩展名和不支持的真实文件类型被拒绝。
- [x] 头像最大 2 MB，其他图片最大 10 MB，超限文件被拒绝。
- [x] 头像每分钟和每日限流生效，事务回滚时限流计数处理符合设计。
- [x] 对象 key 不使用原文件名，采用业务场景、年月和 UUID，不覆盖同名对象。
- [x] 后端返回的公开 URL 使用配置的图片域名，浏览器可以通过 HTTPS 匿名读取。
- [x] 使用受限 RAM 凭据完成一次真实 OSS 上传；前端、响应和日志中不出现 AccessKey Secret。
- [x] 替换头像、封面或删除文章时不会误删 P1 保留的历史 OSS 对象。

## 11. 后台操作审计与安全日志

- [x] 后台新增、更新、删除、状态、角色、审核、回复、批量通过和图片上传等写操作记录审计日志。
- [x] 成功业务与成功审计日志在同一事务提交；业务失败回滚后以独立事务记录失败日志。
- [x] 审计日志按时间和 ID 倒序，可按设计字段组合筛选。
- [x] 审计记录不包含请求体、query 参数、密码、Token、邮箱、评论或留言正文、文件原名。
- [x] 认证成功、失败、改密、禁用、邮件失败和上传失败等结构化日志包含必要事件字段。
- [x] 已匹配用户的登录失败日志记录用户 ID 和规范用户名；未知账号输入经过脱敏。
- [x] 日志中不存在密码、JWT、Refresh Token、SMTP 密码、OSS Secret 等敏感值。

## 12. 前端响应式、主题与可用性

至少检查以下视口：`320px`、`375px`、`768px`、`1024px`、`1440px`。移动端宽度以浏览器实际 CSS viewport 为准。

- [x] 前台顶栏在移动端使用汉堡菜单，导航项完整，头像菜单可用。
- [x] 后台顶栏在移动端显示当前页面标题和汉堡菜单，固定侧栏正确收起。
- [x] 后台筛选表单不会遮挡表格，页面或表格按设计滚动，不产生不可控横向溢出。
- [x] 后台表格移动端不固定操作列，分页器在窄屏下不溢出。
- [x] 登录、注册、留言等输入框在 iOS 风格移动视口下不会因字号过小触发页面自动放大。
- [x] 文章列表、详情、分类、评论、留言和关于页面在各视口无文字或控件重叠。
- [x] 长用户名、长标题、长标签、长 URL、连续字符和大量换行不会撑破容器。
- [x] 浅色、深色、跟随系统三种主题切换和持久化正常。
- [x] 深色模式下表格分隔线、状态标签、对话框、Tooltip、输入框具备足够对比度。
- [x] 键盘 Tab 可以定位主要按钮、链接和菜单，焦点样式可见。
- [x] 加载、空数据、错误、禁用和提交中状态文案明确，不会重复提交。
- [x] Chrome 和 Edge 完成主流程回归；条件允许时在移动 Safari 或真实 iPhone 上检查输入和 Cookie 行为。
...
- [x] 公开写接口应用层限流已验证；生产式部署同时验证 Nginx 边缘限流。
...
- [x] 前端和后端均能通过可复现的 Dockerfile 构建生产镜像。
- [x] Docker Compose 可以启动 Nginx、后端、PostgreSQL、Redis，服务健康检查正常。
- [x] 后端使用 `prod` profile，生产变量通过 `.env`、Compose 或 Secrets 注入，镜像中没有真实密钥。
- [x] 空生产数据库通过一次性 bootstrap 创建首个管理员；密码未进入环境变量、镜像或日志，重复执行会失败。
- [x] Nginx 托管前端静态资源，并将 `/api/**` 反向代理到后端。
- [x] 使用本地域名和可信测试证书或明确记录的临时证书验证 HTTPS。
- [x] HTTPS 下 Refresh Token Cookie 带 `Secure`，登录、刷新、退出流程正常。
- [x] Nginx 正确覆盖并传递客户端地址和 `X-Forwarded-Proto`，后端不信任客户端伪造的代理头。
- [x] Nginx 对留言等公开写接口的请求限流生效。
- [x] 容器重启后 PostgreSQL 数据和 Redis 既定持久化数据符合预期。
- [x] 单个服务更新、停止和重新启动不会误删数据库卷。
- [x] 应用、Nginx、PostgreSQL、Redis 日志可查看，日志中无敏感信息。
- [x] 在该环境重新执行第 17 节部署后冒烟测试。

## 17. 部署后冒烟测试

本节在每次生产式部署后执行。它比全量验收短，但仍有固定标准和结果记录。

- [x] HTTPS 首页、文章列表和文章详情可访问，静态资源无 404。
- [x] 注册、登录、刷新和退出正常，Cookie 属性符合当前环境。
- [x] 管理员进入后台并创建、发布一篇验收文章，前台可见。
- [x] 关键词搜索可以找到该文章，中文全文检索正常。
- [x] 用户发表评论，管理员审核后前台可见。
- [x] 游客发表留言，管理员审核并回复，真实 SMTP 通知到达验收邮箱。
- [x] 上传头像或文章测试图片，真实 OSS URL 可以通过 HTTPS 访问。
- [x] 审计日志中存在本次后台写操作，且不包含敏感内容。
- [x] 重启后端容器后重新访问文章、登录和搜索，数据保持正常。
- [x] 检查各容器日志，无持续异常、重复重启或连接失败。

## 18. 真实服务器部署（正式上线前必做）

建议先完成第 16 节本机生产式部署，再租用或准备服务器。服务器阶段主要验证公网网络、域名、证书、安全组、系统资源、远程备份和真实运维，不应承担第一次发现 Dockerfile 或 Compose 基础错误的成本。

- [N/A] 服务器 CPU、内存、磁盘和带宽满足预期，操作系统和 Docker 版本已记录。（本次不进行正式公网部署。）
- [N/A] 防火墙 / 安全组只开放 SSH、HTTP、HTTPS 等必要端口，数据库和 Redis 不直接暴露公网。（本次不进行正式公网部署。）
- [N/A] 域名解析正确，HTTPS 证书有效并具备续期方案。（本次不进行正式公网部署。）
- [N/A] 使用受限生产凭据连接数据库、Redis、OSS 和 SMTP，Secrets 不进入仓库或镜像。（本次不进行正式公网部署。）
- [N/A] 数据库定时备份已配置到异机或对象存储，并完成至少一次恢复验证。（本次不进行正式公网部署。）
- [N/A] 容器设置合理的重启策略、日志轮转和资源监控。（本次不进行正式公网部署。）
- [N/A] 部署后执行第 17 节冒烟测试。（本次不进行正式公网部署；本机演练已执行第 17 节。）
- [N/A] 监控 CPU、内存、磁盘、容器状态、错误日志和备份失败，具备基础告警方式。（本次不进行正式公网部署。）
...
- [x] 所有自动化质量门禁通过。
- [x] 已执行的验收前冒烟、核心业务流程和本机生产式部署通过。
- [x] 数据库空库迁移、历史升级和恢复演练通过。
- [x] 本机生产式部署与部署后冒烟通过。
- [N/A] 真实服务器部署状态已经明确记录：本次不进行正式公网部署，留待上线准备阶段。
- [x] 已发现缺陷均有严重级别、处理结论和复测记录；本次新增修复提交需先通过 CI 再更新最终验收 SHA。

结论说明：

```text
本次 P1 已完成自动化质量门禁、数据库迁移/备份恢复、本机生产式部署和部署后冒烟。
正式公网部署不纳入本次范围。第 8 至 12 节及第 14.9 节仍有未勾选的逐项证据，
后续若要将“全量清单全部通过”作为正式发布门槛，必须补齐这些场景并更新验收 Commit SHA。
```

| 角色        | 姓名   | 日期       | 结论                         |
| ----------- | ------ | ---------- | ---------------------------- |
| 开发 / 验收 | sanjuu | 2026-09-25 | 本机 P1 验收完成；公网部署待上线准备。 |
