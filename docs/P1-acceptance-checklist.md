...
| 项目                  | 记录                                                         |
| --------------------- | ------------------------------------------------------------ |
| 验收版本 / Commit SHA | 6c58e507a7a8f68e586828e630413dd7bda2de2e                     |
| 验收分支              | `develop`                                                    |
| 验收开始时间          | 2026-09-16                                                   |
| 验收完成时间          |                                                              |
| 验收负责人            | sanjuu                                                       |
...
| 操作系统 / 设备       | Windows 11                                                   |
| 关联 CI 运行地址      | [CI #13](https://github.com/sanjuu2024/blog/actions/runs/35116990403) |
| 最终结论              | 进行中                                                       |
...
| ------ | -------- | ------------ | ------------------------------------------------------------ | ---------------- | ---------------- |
| P1-001 | Minor    | 后台用户管理 | 发现版本 `6c58e507a7a8f68e586828e630413dd7bda2de2e` 存在问题：用户名和邮箱筛选使用大小写敏感匹配，与登录、查重规则不一致。<br />输入 `HEFA` 无法匹配已注册用户名 `hefa`。 | 处理结果：已改为不区分大小写的字面量包含查询 | 复测结果：待复测 |
...
- [ ] 重复用户名返回用户名已存在，重复邮箱返回邮箱已存在。
- [ ] 用户名和邮箱两种方式均可登录，大小写差异不影响账号匹配。
- [ ] 错误密码、未知账号和禁用账号无法登录，错误信息不泄露密码或敏感细节。
- [ ] 登录、注册表单按 Enter 与点击提交按钮行为一致，无效表单不会发送请求。

### 6.2 Token 与会话

- [ ] 登录成功后 Access Token 可访问受保护接口，Refresh Token 写入 HttpOnly Cookie。
- [ ] Access Token 到期后，前端可以通过 Refresh Token 恢复登录态。
- [ ] 刷新成功后旧 Refresh Token 失效，新 Refresh Token 生效。
- [ ] Refresh Token 缺失、伪造、过期、撤销时，刷新失败并清理前端登录态。
- [ ] 退出登录后 Refresh Token 会话失效；重复退出仍返回成功。
- [ ] 修改密码后，旧 Access Token 和全部活跃 Refresh Token 失效。
- [ ] 管理员禁用用户后，该用户旧 Access Token 和 Refresh Token 均失效。
- [ ] 管理员修改用户角色后，旧 Access Token 失效；刷新或重新登录后取得新角色。
- [ ] Redis 缓存缺失时 tokenVersion 可以回源数据库；Redis 数据不作为持久化权威来源。

### 6.3 个人资料与后台用户管理

- [ ] 用户可以查看公开资料和自己的完整资料，公开资料不返回邮箱等私有字段。
- [ ] 用户可以独立修改昵称、简介，未提交的字段保持原值。
- [ ] 用户名不可修改。
- [ ] 用户可以上传头像，刷新后仍显示新头像。
- [ ] 管理员可以按用户名、邮箱、角色、状态组合筛选用户。
- [ ] 管理员不能修改自己的角色或状态。
- [ ] 用户列表、个人资料和安全操作不存在越权访问。
